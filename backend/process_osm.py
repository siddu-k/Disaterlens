...
with open("real_osm_mumbai.json", "r", encoding="utf-8") as f:
...
print(f"Processed {len(roads)} real OSM roads and {len(facilities)} facilities.")
...
out_path = os.path.join(os.path.dirname(__file__), "processed_mumbai_osm.json")
with open(out_path, "w", encoding="utf-8") as f:
    json.dump(data, f, indent=2)

print(f"Saved {len(roads)} roads, {len(facilities)} facilities, and {len(buildings[:2000])} buildings to {out_path}!")
